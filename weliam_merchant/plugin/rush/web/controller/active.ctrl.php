...
class Active_WeliamController
{
	public function activelist()
	{
		...
		foreach ($activity as $key => &$value) {
			...
			$value['placeorder'] = pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('wlmerchant_rush_order') . (' WHERE activityid = ' . $value['id'] . ' AND status in (0,1,2,3,4,6,8,9)'));

			if (empty($value['placeorder'])) {
				$value['placeorder'] = 0;
			}

			$value['alreadypay'] = pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('wlmerchant_rush_order') . (' WHERE activityid = ' . $value['id'] . ' AND status in (1,2,3,4,6,8,9)'));

			if (empty($value['alreadypay'])) {
				$value['alreadypay'] = 0;
			}

			$value['alreadyuse'] = pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('wlmerchant_rush_order') . (' WHERE activityid = ' . $value['id'] . ' AND status in (2, 3)'));
		...
		$status4 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename(PDO_NAME . 'rush_activity') . (' WHERE uniacid=' . $_W['uniacid'] . ' and status=4 and aid=' . $_W['aid']));
	...
	public function createactive()
	{
		...
		if ($id) {
			...
			$allspecs = pdo_fetchall('select * from ' . tablename('wlmerchant_goods_spec') . ' where goodsid=:id AND type = 1 order by displayorder asc', array(':id' => $id));
			...
			unset($s);
			$html = '';
			$options = pdo_fetchall('select * from ' . tablename('wlmerchant_goods_option') . ' where goodsid=:id and type = 1 order by id asc', array(':id' => $id));
			$specs = array();
			...
			if (0 < count($options)) {
				...
				$html .= '<th><div class=""><div style="padding-bottom:10px;text-align:center;">抢购价</div><div class="input-group"><input type="text" class="form-control input-sm option_price_all"  VALUE=""/><span class="input-group-addon" ><a href="javascript:;" class="fa fa-angle-double-down" title="批量设置" onclick="setCol(\'option_price\');"></a></span></div></div></th>';
		...
		$specials = pdo_getall('wlmerchant_rush_special', array('uniacid' => $_W['uniacid'], 'aid' => $_W['aid']));
		...
		if ($_W['ispost']) {
			...
			if ($goods['price'] < $goods['creditmoney'] || $goods['creditmoney'] == $goods['price']) {
				wl_message('积分不能全额抵扣');
			}
			...
			if (5 < $goods['allsalenum']) {
				$allsalenum = 5;
			}
			else {
				$allsalenum = $goods['allsalenum'];
			}

			if ($falsenum < $allsalenum) {
				$needorder = $allsalenum - $falsenum;
				$needorder = intval($needorder);
				$data = util_csv::read_csv_lines('../addons/weliam_merchant/web/resource/download/robotnickname.csv', 5000, 0);
				$nicknames = array();

				foreach ($data as $key => &$da) {
					$da[0] = trim(iconv('gb2312', 'utf-8', $da[0]));
				...
				$url = '../addons/weliam_merchant/web/resource/images/head_imgs';
				$head_imgs_array = Tools::get_head_img($url, $needorder);
				$k = 0;

				while ($k < $needorder) {
					$nickid = rand(0, count($nicknames) - 1);
					$falseorder['nickname'] = $nicknames[$nickid];
					$falseorder['avatar'] = $head_imgs_array[$k];
					$falseorder['time'] = time();
					$falseorders[] = $falseorder;
					++$k;
				}
			}
			else {
				if ($allsalenum < $falsenum) {
					$falseorders = array_slice($falseorders, 0, $allsalenum);
				}
			}

			$goods['falseorder'] = serialize($falseorders);
			...
			if ($goods['optionstatus']) {
				...
				if (0 < count($specids)) {
					pdo_query('delete from ' . tablename('wlmerchant_goods_spec') . ' where  type = 1 and goodsid=' . $id . ' and id not in (' . implode(',', $specids) . ')');
				}
				else {
					pdo_query('delete from ' . tablename('wlmerchant_goods_spec') . ' where  type = 1 and goodsid=' . $id);
				}
				...
				if (0 < count($optionids)) {
					pdo_query('delete from ' . tablename('wlmerchant_goods_option') . ' where type = 1 AND goodsid=' . $id . ' and id not in ( ' . implode(',', $optionids) . ')');
				}
				else {
					pdo_query('delete from ' . tablename('wlmerchant_goods_option') . ' where type = 1 AND goodsid=' . $id);
				}
			}

			wl_message('保存成功！', web_url('rush/active/activelist'), 'success');
		}
	...
	public function spec()
	{
		...
		include wl_template('active/spec');
	}
	...
	public function spec_item()
	{
		...
		include wl_template('active/spec_item');
	}
	...
	public function rushList()
	{
		...
		$sql = 'SELECT ' . $field . ' FROM ' . tablename(PDO_NAME . 'rush_activity') . ' a LEFT JOIN ' . tablename(PDO_NAME . 'rush_category') . ' b ON a.cateid = b.id LEFT JOIN ' . tablename(PDO_NAME . 'merchantdata') . ' m ON a.sid = m.id';
		...
		foreach ($data['list'] as $k => &$v) {
			...
			$v['order_purchase'] = $orderModel->getPurchaseQuantity($orderW . ' (0,1,2,3,4,6,8,9) ', true) ?: 0;
			$v['order_payment'] = $orderModel->getPurchaseQuantity($orderW . ' (1,2,3,4,6,8,9) ', true) ?: 0;
			$v['order_used'] = $orderModel->getPurchaseQuantity($orderW . ' (2,3) ', true) ?: 0;
