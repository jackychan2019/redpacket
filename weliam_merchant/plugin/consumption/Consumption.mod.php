...
class Consumption
{
	static public function creditshop_adv_get($status = -1)
	{
		global $_W;
	...
	static public function creditshop_can_exchange_goods($idOrGoods, $uid = '')
	{
		global $_W;
		$goods = $idOrGoods;
	...
	static public function creditshop_category_get($status = -1)
	{
		global $_W;
		$condition = ' where uniacid = :uniacid';
		$params = array(':uniacid' => $_W['uniacid']);
	...
	static public function creditshop_goodsall_get($filter = array())
	{
		global $_W;
		global $_GPC;
	...
	static public function creditshop_goods_get($goods_id)
	{
		global $_W;
	...
	static public function creditshop_record_get($filter = array())
	{
		global $_W;
		global $_GPC;
	...
	static public function creditshop_order_get($id)
	{
		global $_W;
	...
	static public function creditshop_orderall_get($filter = array())
	{
		global $_W;
		global $_GPC;
	...
	static public function creditshop_order_update($orderOrId, $type, $extra = array())
	{
		global $_W;
		$order = $orderOrId;
	...
	static public function payconsumOrderNotify($params)
	{
		global $_W;
		...
		$data = self::getPayData($params, $order_out);
		...
		if ($order_out['status'] == 0) {
			...
			$res = Member::credit_update_credit1($order_out['mid'], 0 - $activeInfo['use_credit1'], '兑换[' . $activeInfo['title'] . ']消耗');

			if ($res) {
				if ($activeInfo['type'] == 'credit2') {
					$res2 = Member::credit_update_credit2($order_out['mid'], $activeInfo['credit2'], '兑换[' . $activeInfo['title'] . ']获得余额');
				}
				else {
					if ($activeInfo['type'] == 'halfcard') {
						$res2 = self::conhalfcard($order_out['mid'], $activeInfo['halfcardid'], $order_out['name']);
					}
				}

				if ($res) {
					$recorddata = array('uniacid' => $order_out['uniacid'], 'mid' => $order_out['mid'], 'goodsid' => $activeInfo['id'], 'orderid' => $order_out['id'], 'createtime' => time(), 'status' => $status, 'integral' => $activeInfo['use_credit1'], 'money' => $activeInfo['use_credit2'], 'expressid' => $order_out['expressid']);
	...
	static public function getPayData($params, $order_out)
	{
	...
	static public function payconsumOrderReturn($params, $backurl = false)
	{
		...
		wl_message('支付成功', app_url('order/userorder/payover', array('id' => $order_out['id'], 'type' => 9)), 'success');
	}

	static public function conhalfcard($mid, $halfcardid, $username)
	{
		global $_W;
		...
		if ($halfcard) {
			...
			$halfcarddata = array('uniacid' => $_W['uniacid'], 'aid' => $halfcard['aid'], 'mid' => $mid, 'expiretime' => $limittime, 'username' => $username, 'levelid' => $halfcard['levelid'], 'createtime' => time());
