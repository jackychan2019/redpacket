...
class Fightgoods_WeliamController
{
	public function ptgoodslist()
	{
		...
		$status0 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename(PDO_NAME . 'fightgroup_goods') . (' WHERE uniacid=' . $_W['uniacid'] . ' and status=0 and aid=' . $_W['aid']));
		$status4 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename(PDO_NAME . 'fightgroup_goods') . (' WHERE uniacid=' . $_W['uniacid'] . ' and status=4 and aid=' . $_W['aid']));
		...
		if (!empty($_GPC['keyword'])) {
			if (!empty($_GPC['keywordtype'])) {
				...
				case 1:
					$wheres['@id@'] = $_GPC['keyword'];
					break;

				case 2:
					$wheres['@merchantid@'] = $_GPC['keyword'];
					break;
		...
		$goodslist = Wlfightgroup::getNumGoods('*', $wheres, 'listorder DESC', $pindex, $psize, 1);
		$pager = $goodslist[1];
		$list = $goodslist[0];
		...
		foreach ($list as $key => &$v) {
			...
			$v['placeorder'] = pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('wlmerchant_order') . (' WHERE fkid = ' . $v['id'] . ' AND plugin = \'wlfightgroup\' AND status in (0,1,2,3,4,6,8,9)'));

			if (empty($v['placeorder'])) {
				$v['placeorder'] = 0;
			}

			$v['alreadypay'] = pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('wlmerchant_order') . (' WHERE fkid = ' . $v['id'] . ' AND plugin = \'wlfightgroup\' AND status in (1,2,3,4,6,8,9)'));

			if (empty($v['alreadypay'])) {
				$v['alreadypay'] = 0;
			}

			$v['alreadyuse'] = pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('wlmerchant_order') . (' WHERE fkid = ' . $v['id'] . ' AND plugin = \'wlfightgroup\' AND status in (2, 3)'));

			if (empty($v['alreadyuse'])) {
				$v['alreadyuse'] = 0;
			}
		}
	...
	public function creategood()
	{
		...
		if ($_W['ispost']) {
			...
			if ($goods['specstatus']) {
				...
				while ($k < $len) {
					...
					$a = array('uniacid' => $_W['uniacid'], 'stock' => $optionArray['option_stock'][$k], 'title' => $optionArray['option_title'][$k], 'price' => $optionArray['option_price'][$k], 'vipprice' => $optionArray['option_vipprice'][$k], 'settlementmoney' => $optionArray['option_settlementmoney'][$k], 'onedismoney' => $optionArray['option_onedismoney'][$k], 'twodismoney' => $optionArray['option_twodismoney'][$k], 'threedismoney' => $optionArray['option_threedismoney'][$k], 'goodsid' => $id, 'specs' => $newids, 'type' => 2);
					$totalstocks += $a['stock'];
					$pricearray[] = $a['price'];
		...
		include wl_template('ptgoods/creategood');
	}
	...
	public function categorylist()
	{
		...
		$wheres['uniacid'] = $_W['uniacid'];
		$wheres['aid'] = $_W['aid'];
		$categorys = Wlfightgroup::getNumCategory('*', $wheres, 'listorder DESC', $pindex, $psize, 1);
		$pager = $categorys[1];
		$list = $categorys[0];
		include wl_template('ptgoods/ptcategorylist');
	}
	...
	public function createcategory()
	{
		...
		if ($id) {
			$category = Wlfightgroup::getCategory($id);
		}
		...
		if (checksubmit('submit')) {
			...
			if ($id) {
				$res = Wlfightgroup::updateCategory($category, $id);

				if ($res) {
					wl_message('更新分类成功', web_url('wlfightgroup/fightgoods/categorylist'), 'success');
				}
				else {
					wl_message('更新分类失败', referer(), 'error');
				}
			}
			else {
				$res = Wlfightgroup::saveCategory($category);

				if ($res) {
					wl_message('创建分类成功', web_url('wlfightgroup/fightgoods/categorylist'), 'success');
				}
				else {
					wl_message('创建分类失败', referer(), 'error');
				}
			}
		}

		include wl_template('ptgoods/createcategory');
	}
	...
	public function deleteCategory()
	{
		...
		global $_GPC;
		$id = $_GPC['id'];
		$res = Wlfightgroup::deteleCategory($id);
	...
	public function fightList()
	{
		...
		-1 < $_GPC['cate_id'] && ($where .= ' AND a.categoryid = ' . $_GPC['cate_id'] . ' ');
		!empty($_GPC['shop_id']) && ($where .= ' AND a.merchantid = ' . $_GPC['shop_id'] . ' ');
		...
		$sql = 'SELECT ' . $field . ' FROM ' . tablename(PDO_NAME . 'fightgroup_goods') . ' a LEFT JOIN ' . tablename(PDO_NAME . 'fightgroup_category') . ' b ON a.categoryid = b.id LEFT JOIN ' . tablename(PDO_NAME . 'merchantdata') . ' m ON a.merchantid = m.id';
	...
	public function toExamine()
	{
		...
		if ($flag) {
			$res = Wlfightgroup::updateGoods(array('status' => 1), $id);
		}
		else {
			$res = Wlfightgroup::updateGoods(array('status' => 6), $id);
		}
	...
	public function updateInfo()
	{
		...
		if ($res) {
			if (in_array($_GPC['field'], array('price', 'aloneprice', 'oldprice'))) {
				show_json(1, array('message' => '修改成功', 'data' => sprintf('%.2f', $_GPC['value'])));
			}
