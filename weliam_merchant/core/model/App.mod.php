	...
	static public function getPlugins($type = 3)
	{
		...
		foreach ($styles as $key => $value) {
			...
			if (!empty($config) && is_array($config)) {
				...
				if (empty($plugininfo)) {
					pdo_insert('wlmerchant_plugin', array('name' => $config['ident'], 'type' => $config['category'], 'title' => $config['name'], 'thumb' => '../addons/weliam_merchant/plugin/' . $config['ident'] . '/icon.png', 'ability' => $config['des'], 'status' => 1));
				}
				else {
